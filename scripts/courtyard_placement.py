...
PITCH=.25;W=45.;H=42.;GAP=.05;EDGE=.40
...
def main():
 ...
 forced_bottom=set(['U1','U2','Q1','D2','D1','R2','L1','L2','R1','R3','R4','R7','R8','R9','R10']+[c for c,u in caps.items() if u in ('U1','U2')])
 ...
 while unplaced:
  def priority(ref):
   ...
   return (5000 if ref in ('Y1','Y2','L4','L3') else 0)+(3000 if ref in special_hosts and special_hosts[ref] in placed else 0)+(1000 if ref in anchor_centers else 0)+(100 if area>9 else 0)+connected*4+area
 ...
 svg.append('</svg>');(ROOT/'reports/placement-45x42-courtyards.svg').write_text(''.join(svg))
