...
import subprocess
PITCH=.25;W=45.;H=37.;GAP=.20;EDGE=.40

def main():
 original_board_text=subprocess.check_output(['git','show','7210bdb:pcbgolf.kicad_pcb'],cwd=ROOT,text=True,encoding='utf-8')
 ...
 source_board=sx.loads(original_board_text)
 ...
 put('U3',15.6,20.7,0,'B.Cu')
 for ref,x in zip(['J5','J6','J7','J8'],[3.25,10.75,18.25,25.75]):put(ref,x,4.75,90)
 put('J1',39.5,16.,180,by_origin=True) # open barrel faces right edge
 put('J3',41.325,31.,90,by_origin=True) # USB-C mouth exactly at bottom outline
 put('J4',38.,4.)
 put('SW1',6.5,32.5)
 ...
 anchor_centers={'U1':(33,21),'U2':(39,21),'U5':(17,27),'U6':(22,27),'U7':(27,27),'U8':(32,27),'U9':(7,9),'U10':(17,9),'U11':(28,9),'U12':(39,9),'U13':(4,11),'U14':(12,11),'U15':(20,11),'U16':(27,11)}
 ...
 source=sx.loads(original_board_text)
 ...
 # Mating cable envelopes: larger than USB-IF12.35x6.5mm maximum.
 plug_envelopes=[]
 for ref in ('J5','J6','J7','J8'):
  pp=placed[ref];x,y=pp['x'],pp['y'];plug_envelopes.append({'ref':ref,'bounds':[x-3.5,y-6.25,x+3.5,y+6.25],'width_mm':7.,'length_mm':12.5,'overmold_above_pcb_mm':8.6})
 plug_conflicts=[]
 for a,b in itertools.combinations(plug_envelopes,2):
  aa,bb=a['bounds'],b['bounds']
  if min(aa[2],bb[2])>max(aa[0],bb[0]) and min(aa[3],bb[3])>max(aa[1],bb[1]):plug_conflicts.append([a['ref'],b['ref']])
 # Tall barrel model is the only non-USB component crossing the overmold plane.
 barrel_model=[29.6,16.-6.45,44.5,16.+6.15]
 for a in plug_envelopes:
  aa=a['bounds'];bb=barrel_model
  if min(aa[2],bb[2])>max(aa[0],bb[0]) and min(aa[3],bb[3])>max(aa[1],bb[1]):plug_conflicts.append([a['ref'],'J1'])
 ...
 svg.append('</svg>');(ROOT/'reports/placement-45x37-mating.svg').write_text(''.join(svg))
