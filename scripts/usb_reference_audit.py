...
z=json.loads((u.ROOT/'reports/usb-filled-zones.json').read_text())
...
(u.ROOT/'reports/usb-reference-audit.json').write_text(json.dumps(result,indent=2));print(json.dumps({k:v for k,v in result.items() if k not in ('all_tracks','return_vias')},indent=2))
