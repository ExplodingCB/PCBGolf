...
def main():
 ...
 while unplaced:
  def priority(ref):
   ...
   return (5000 if ref in ('Y1','Y2','L4','L3') else 0)+(3000 if ref in special_hosts and special_hosts[ref] in placed else 0)+(2000 if ref in ('L5','L6','L7','L8','D1') else 0)+(1000 if ref in anchor_centers else 0)+(100 if area>9 else 0)+connected*4+area
 ...
 svg.append('</svg>');(ROOT/'reports/placement-45x37-topology.svg').write_text(''.join(svg))
